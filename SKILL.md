...
- 가져온 이메일/이름은 이후 모든 단계에서 `MY_EMAIL`, `MY_NAME`으로 참조한다 (하드코딩 금지).
...
- **참조 수신 (Cc):** 우선순위를 한 단계 낮춰 판단
  ...
  - 단, Cc라도 아래 조건 중 하나라도 해당하면 강등 없이 원래 등급 유지:
    - 메일 본문/제목에 `MY_NAME`(또는 이름 일부)이 직접 언급된 경우
...
- `MY_NAME` 또는 `MY_EMAIL`을 간접적으로 지칭하며 결정을 요청하는 표현이 있는 경우 (예: "팀장님께 확인", "담당자 의견" 등)
