...
- 본인 이메일은 `getProfile`로 가져온다. 실패 시 사용자에게 직접 묻는다.
...
- **참조 수신 (Cc):** 우선순위를 한 단계 낮춰 판단
  - High 해당 → Medium으로 강등
  - Medium 해당 → Low로 강등
  - 단, Cc라도 본인이 명시적으로 지목되거나("정유진 팀장님" 등 이름 언급) 직접 답변 요청이 있으면 강등 없이 원래 등급 유지

#### 👀 Cc 중요 의사결정 마킹

Cc로 수신된 메일이라도 아래 조건에 해당하면 분류 등급과 무관하게 `👀 확인 필요` 태그를 추가한다:

- 본인이 의사결정 체인에 포함된 경우 (예: 팀장으로서 최종 승인 또는 방향 결정 필요)
- 계약, 파트너십, 예산, 인사 관련 내용이 포함된 경우
- 본인 팀/업무 영역의 중요한 방향 전환이나 전략 결정이 논의되는 경우
- "정유진 팀장님께 확인", "팀장님 의견" 등 간접적으로 본인 결정을 요청하는 표현이 있는 경우
